  ...
  const executeSearch = useCallback(async (rawQuery) => {
    ...
    try {
      ...
      if (data && data.results && data.results.length > 0) {
        ...
        setErrorMessage(`No titles found for "${query}".`);
      }
    } catch {
      setMovies(INITIAL_POPULAR);
      setErrorMessage("Network issue detected.");
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100%', maxWidth: '100vw', overflowX: 'hidden', display: 'flex', flexDirection: 'column', background: '#070b13', color: '#f8fafc' }}>
      ...
      <main className="container" style={{ flex: 1 }}>
        ...
        {/* Dynamic Netflix-Style Hero Spotlight (Replacing Static CineTrack Text) */}
        ...
        <section>
          ...
          {movies.length === 0 && activeTab === 'explore' ? (
            ...
            <div style={{ textAlign: 'center', padding: '3.5rem 1rem', color: '#94a3b8', background: 'rgba(255,255,255,0.02)', borderRadius: '12px' }}>
              ...
              <p style={{ fontSize: '0.76rem', color: '#94a3b8', margin: '4px 0 14px 0' }}>Explore trending cinema and save movies to watch later.</p>
              <button onClick={() => setActiveTab('explore')} className="btn-primary" style={{ fontSize: '0.76rem' }}>
                Explore Trending Cinema
              </button>
            </div>
          ) : (
            <div className="card-grid">
              {(activeTab === 'explore' ? movies : watchlist).map((movie, idx) => (
